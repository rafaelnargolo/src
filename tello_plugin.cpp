...
namespace tello_gazebo {
  ...
  double init_Rotate = 0.0;
  double delta_Rotate = 0.0;
    ...
    void transition(FlightState next) {
      ...
      switch (flight_state_) {
        ...
        case FlightState::landing:
          ...
          break;
        case FlightState::moving:
          if (status == "up") {
            set_target_velocities(0, 0, 0.5, 0);
            break;
          } else if (status == "down") {
            set_target_velocities(0, 0, -0.5, 0);
            break;
...
#pragma ide diagnostic ignored "UnusedValue"
    ...
    void command_callback(
      ...
      if (request->cmd == "takeoff" && flight_state_ == FlightState::landed) {
        ...
        response->rc = response->OK;
      } else if (is_prefix("up", request -> cmd) && flight_state_ == FlightState::flying) {
        up(request -> cmd);
        transition(FlightState::moving);
        response -> rc = response -> OK;
      } else if (is_prefix("down", request -> cmd) && flight_state_ == FlightState::flying) {
        down(request -> cmd);
        transition(FlightState::moving);
      }
    ...
    void up(std::string up_command) {
      std::istringstream iss(up_command, std::istringstream::in);
      std::string s;
      iss >> s; // Pula string 'up'
      iss >> s;
      const float x = std::stof(s);
      RCLCPP_WARN(node_->get_logger(), "Subindo... Valor: %f", x);
      ignition::math::Pose3d pose = base_link_->WorldPose();
      init_Z = pose.Pos().Z();
      delta_Z = x;
      status = "up";
    ...
    void down(std::string down_command) {
      std::istringstream iss(down_command, std::istringstream::in);
      std::string s;
      iss >> s; // Pula string 'down'
      iss >> s;
      const int x = std::stof(s);
      RCLCPP_WARN(node_->get_logger(), "Descendo... Valor: %d", x);
      ignition::math::Pose3d pose = base_link_ -> WorldPose();
      init_Z = pose.Pos().Z();
      delta_Z = x;
      status = "down";
    ...
    void left(std::string left_command) {
      std::istringstream iss(left_command, std::istringstream::in);
      std::string s;
      iss >> s; // Pula a string 'left'
      iss >> s;
      const int x = std::stof(s);
      RCLCPP_WARN(node_->get_logger(), "Indo para esquerda... Valor: %d", x);
      ignition::math::Pose3d pose = base_link_ -> WorldPose();
      init_Y = pose.Pos().Y();
      delta_Y = x;
    }

    void right(std::string right_command) {
      std::istringstream iss(right_command, std::istringstream::in);
      std::string s;
      iss >> s; // Pula a string 'right'
      iss >> s;
      const int x = std::stof(s);
      RCLCPP_WARN(node_->get_logger(), "Indo para direita... Valor: %d", x);
      ignition::math::Pose3d pose = base_link_ -> WorldPose();
      init_Y = pose.Pos().Y();
      delta_Y = x;
    }

    void forward(std::string forward_command) {
      std::istringstream iss(forward_command, std::istringstream::in);
      std::string s;
      iss >> s; // Pula a string 'forward'
      iss >> s;
      const int x = std::stof(s);
      RCLCPP_WARN(node_->get_logger(), "Indo para frente... Valor: %d", x);
      ignition::math::Pose3d pose = base_link_ -> WorldPose();
      init_X = pose.Pos().X();
      delta_X = x;
    ...
    void back(std::string back_command) {
      std::istringstream iss(back_command, std::istringstream::in);
      std::string s;
      iss >> s; // Pula a string 'back'
      iss >> s;
      const int x = std::stof(s);
      RCLCPP_WARN(node_->get_logger(), "Indo para trás... Valor: %d", x);
      ignition::math::Pose3d pose = base_link_ -> WorldPose();
      init_X = pose.Pos().X();
      delta_X = x;
    }

    void rotateRight(std::string rotateR_command) {
      std::istringstream iss(rotateR_command, std::stringstream::in);
      std::string s;
      iss >> s; // Pula o 'cw'
      iss >> s;
      const int x = std::stof(s);
      RCLCPP_WARN(node_ -> get_logger(), "Girando sentido horário... Valor: %d", x);
      ignition::math::Pose3d pose = base_link_ -> WorldPose();
      init_Rotate = pose.Rot().X();
      delta_Rotate = x;
...
#pragma clang diagnostic pop
    ...
    void spin_10Hz() {
      ...
      if (flight_state_ == FlightState::taking_off && pose.Pos().Z() > TAKEOFF_Z) {
        ...
        transition(FlightState::landed);
        respond_ok();
      } else if (flight_state_ == FlightState::moving && pose.Pos().Z() > init_Z + delta_Z && status == "up") {
        transition(FlightState::flying);
        respond_ok();
      } else if (flight_state_ == FlightState::moving && pose.Pos().Z() < init_Z - delta_Z && status == "down") {
        transition(FlightState::flying);
        respond_ok();
      }
